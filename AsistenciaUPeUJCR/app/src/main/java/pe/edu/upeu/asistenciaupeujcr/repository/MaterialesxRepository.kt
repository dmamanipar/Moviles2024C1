...
class MaterialesxRepositoryImp @Inject constructor(
    ...
    }override fun reportarMaterialesxes():
            LiveData<List<MaterialesxConActividad>> {
        try {
            CoroutineScope(Dispatchers.IO).launch{
                delay(3000)
                val
                        data=restMaterialesx.reportarMaterialesx(TokenUtils.TOKEN_CONTENT).body()!!
                val dataxx = data.map {
                    Materialesx(it.id, it.cui,
                        it.tipoCui,it.materEntre,it.fecha,it.horaReg, it.latituda,it.longituda,
                        it.modFh, it.offlinex, it.actividadId.id)
                }
...
//Log.i("DATAXXX", "${materialesx.actividadId}")
        return restMaterialesx.insertarMaterialesx(TokenUtils.TOKEN_CONTENT,
            materialesx).body()!=null
    }
    override suspend fun modificarRemoteMaterialesx(materialesx:Materialesx):Boolean{
        CoroutineScope(Dispatchers.IO).launch {
            Log.i("VER", TokenUtils.TOKEN_CONTENT)
        }
        return restMaterialesx.actualizarMaterialesx(TokenUtils.TOKEN_CONTENT,
            materialesx.id, materialesx).body()!=null
    }
}